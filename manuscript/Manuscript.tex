...
大量学者针对 P2P 网络贷款的投资组合优化问题开展了深入研究。Wan 等人将贷款投资组合决策转化为一个在特定时点下实现收益最大化与风险最小化的优化问题，并引入混合治愈模型以提升投资效果，构建实例驱动的模型对投资者的组合决策进行优化~\cite{wan2023hybrid}. Guo 等人则提出一种基于实例的 P2P 投资组合决策模型，从风险最小角度出发，利用 LendingClub 和 Prosper 数据集实现投资组合配置优化~\cite{guo2016instance}. Ajay 等人则将该问题转化为多目标优化模型，在计算贷款相似度时将期望值框架与传统核方法结合，优化结果优于既有模型\cite{byanjankar2021data}.
